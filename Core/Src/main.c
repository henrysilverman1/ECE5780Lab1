...
int main(void){
		...
		RCC->AHBENR |= RCC_AHBENR_GPIOCEN; // Enable peripheral clock to GPIOA
		...
		GPIOC->MODER |= ((1 <<	12) | (1 <<	14)); //Setting second MODER bits to 1 for general-purpose output mode for PC6 and PC7
		GPIOC->MODER &= ~((1 <<	13) | (1 <<	15)); // Setting first MODER bits to 0 general-purpose output mode for PC6 and PC7
